    ...
    return context => {
        ...
        function visit(node) {
            if (node.kind === ts.SyntaxKind.Identifier && node.id) {
                ...
                while (!(parent && parent.locals instanceof Map && parent.locals.size > 0 && symbolFlag) && parent) {
                ...
                if (type.flags & ts.TypeFlags.Object) {
                    if (type.flags & ts.TypeFlags.Enum || type.flags & ts.TypeFlags.EnumLike || type.flags & ts.TypeFlags.EnumLiteral) {
                    ...
                    for (const prop of properties) {
                        ...
                        fields.push(`${name}${hasQuestionToken ? '?' : ''}: ${propTypeName};`)
                    }
...
const readpath = '' /*'./src/components/CommForm/test.ts'*/
const program = ts.createProgram([resolvePath(readpath)], { emitDeclarationOnly: false })
const result = ts.transpileModule(fs.readFileSync(resolvePath(readpath)).toString(), {
    transformers: { before: [createTransformer(program.getTypeChecker())] }
})

fs.writeFileSync('./src/utils/type-print/print.js', result.outputText)
console.log('编译成功')
